...
def ni(gbest,ibest):
    return(gbest[1]/ibest[1])

def Fni(ni):
    if ni < .0001:
        return 2
    if ni < .01 and ni>= .0001:
        return 1
    if ni < .1 and ni>= .01:
        return .3
    if ni < .9 and ni>= .1:
...
pathGlobal = "/home/mariel/Desktop/TCC/PSO_Niching/" 
    ...
    if len(swarm.keys())> 5:
        for subswarm in swarm.keys():
            ...
            if len(swarm[subswarm]) < 2:
                ...
                #subSA = SA(swarm[subswarm][0])
                newSwarm += swarm[subswarm]
                continue 
...
for subswarm in swarm.keys():
    ...
    pose.dump_pdb(pathGlobal+""+str(protein)+"_"+str(strike)+"/Cluster_"+str(c)+"_"+str(protein)+"_"+str(strike)+".pdb")
    c += 1 
it = 0
for subswarm in swarm.keys():
